...
import { ArrowRight, Check, Copy, ExternalLink, Link2, PackageOpen, QrCode, Search, Settings2 } from "lucide-react";
...
export function ProductManager({ cards }: ProductManagerProps) {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((product) => {
                  return [product.id, product.label, product.type, product.database, product.redirectUrl]
        .join(" ")
        .toLowerCase()
        .includes(search);
    });
  ...
  async function saveRedirectUrl(cardId: string, redirectUrl: string) {
    ...
    try {
      ...
      setProducts((current) =>
        current.map((product) => (product.id === cardId ? payload.card! : product))
      );
    } catch {
  ...
  async function copyRedirect(cardId: string) {
    const value = `${window.location.origin}/api/redirect?id=${cardId}`;
    ...
    return (
      <section className="px-5 py-10 md:py-14">
        <div className="mx-auto max-w-4xl rounded-[2rem] border border-white/10 bg-white/[0.04] p-8 text-center">
          ...
          <h2 className="mt-6 text-3xl font-semibold tracking-tight">No products activated yet.</h2>
          <p className="mx-auto mt-3 max-w-2xl text-sm leading-6 text-white/60">
            This is what a new customer sees before activating their card. Tap or scan the NFC product, enter the activation code, and the product will appear here.
          </p>
          <Link
            href="/activate"
            ...
            {filteredProducts.map((product) => (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.18em] text-pulse">{product.database} database</p>
                ...
                <p className="mt-3 truncate text-sm text-white/48">{product.redirectUrl || "No destination yet"}</p>
              </button>
            ))}
          </div>
        </aside>
          ...
          <div className="glass overflow-hidden rounded-[2rem]">
              ...
              <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
                <div>
                  <div className="flex flex-wrap gap-2">
                    ...
                    <span className="rounded-full border border-white/14 bg-black/18 px-3 py-1 text-xs font-semibold text-white/76">{selectedProduct.database} database</span>
                  </div>
                  <h2 className="mt-5 text-3xl font-semibold tracking-tight md:text-4xl">{selectedProduct.label}</h2>
                  <p className="mt-3 text-sm leading-6 text-white/58">
                    Activation code {selectedProduct.activationCode} · Updated {selectedProduct.updatedAt}
                  </p>
                </div>
                <div className="rounded-3xl border border-white/10 bg-white p-3 text-ink">
                  <QRCodeSVG value={`https://pulsetap.co.uk/api/redirect?id=${selectedProduct.id}`} size={132} />
            ...
            <div className="grid gap-0 md:grid-cols-[1fr_270px]">
              <div className="p-6 md:p-8">
                ...
                <div className="mt-5 rounded-3xl border border-white/10 bg-black/18 p-4">
                  ...
                  <p className="mt-2 break-all text-sm leading-6 text-white/58">
                    https://pulsetap.co.uk/api/redirect?id={selectedProduct.id}
                  </p>
                  <div className="mt-4 flex flex-wrap gap-3">
                    ...
                    <a
                      href={`/api/redirect?id=${selectedProduct.id}`}
                      target="_blank"
                      rel="noreferrer"
              ...
              <div className="border-t border-white/10 p-6 md:border-l md:border-t-0 md:p-8">
                <p className="text-sm font-semibold">Product settings</p>
                <div className="mt-4 space-y-3 text-sm">
                  <div className="rounded-2xl border border-white/10 bg-black/18 p-4">
                    <p className="text-white/46">Status</p>
                    <p className="mt-1 font-semibold">{selectedProduct.activated ? "Active redirect" : "Needs setup"}</p>
                  </div>
                  <div className="rounded-2xl border border-white/10 bg-black/18 p-4">
                    <p className="text-white/46">Total taps</p>
                    <p className="mt-1 font-semibold">{selectedProduct.taps}</p>
                  </div>
                  <div className="rounded-2xl border border-white/10 bg-black/18 p-4">
                    <p className="text-white/46">Access</p>
